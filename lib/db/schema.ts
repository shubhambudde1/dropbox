...
import { relations } from "drizzle-orm";
import { boolean, uuid, text } from "drizzle-orm/pg-core";
...
export const Files = pgTable("files", {
    ...
    // storage information
    fileUrl: text("file_url").notNull(), // URL to access the file
    thubnailUrl: text("thumbnail_url").notNull(), // URL to access the thumbnail\

    // owenership and metadata
    userId: text("user_id").notNull(), // user who owns the file
...
export const fileRelations = relations(Files, ({ one, many }) => ({
    ...
    // rrelationship to file and folder
    children: many(Files)
}));
